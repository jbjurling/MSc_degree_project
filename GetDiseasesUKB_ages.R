...
ind1<-which(BC==1)  ## Beh<C3><B6>ver bara loopa <C3><B6>ver de som har en diagnos.

### Self reported

##Data-Field 20007 Description:    Interpolated Age of participant when cancer first diagnosed
indC<-which(strtrim(names(temp),7) %in% c("f.20001") )  ### diagnos filed i UKB
...
BC_age_SR[which(!(BC_age_SR>0&BC_age_SR<100))]<-NA  ## N<C3><A5>gra har konstiga v<C3><A4>rden
...
for(i in ind1){   ## Loop over particpants with registrty data
    for(j in nc){    ## Det finns olika diagnoser med olika antal bokst<C3><A4>ver ###
    ind<-which(strtrim(as.matrix(temp[i,indC9]),j) %in% names)     ## Vilken ICD..9 matchar diganoserna
   ind<-c(ind,which(strtrim(as.matrix(temp[i,indC10]),j) %in% names)) ## Vilken ICD..10 matchar diganoserna
   if(NROW(ind)>0){BC_age_CR[i]<-min(temp[i, indA[ind]], na.rm=T)}   ## V<C3><A4>lj den l<C3><A4>gst <C3><A5>ldern som matchar diagnosen
}}


print(paste("There was",  NROW(which(!is.na(BC_age_CR))),  "ages identified in the Cancer registry"))

## Cause and age at death  -  Likadan som cancers med en anna variable f<C3><B6>r <C3><A5>ldern
...
### 41202/41204/dates of first diagnose (main and secondary <C3><A4>r hopslagna)
indC10<-which(strtrim(names(temp),7) %in% c("f.41270") )  ### ICD10 Data fields i UK biobank.
indD10<-which(strtrim(names(temp),7) %in% c("f.41280") )  ### Date first dianose
